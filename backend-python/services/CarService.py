...
async def lookup_vehicle(registration_number: str) -> Optional[CarBase]:
    """
    Lookup vehicle information from Norwegian Vehicle Registry API
    """
    if not VEHICLE_API_KEY:
        raise ValueError("Missing VEHICLE_API_KEY in environment variables")
    
    headers = {
        "SVV-Authorization": f"Apikey {VEHICLE_API_KEY}",
    ...
    async with httpx.AsyncClient() as client:
        ...
        teknisk_data = kjoretoy_data.get("godkjenning", {}).get("tekniskGodkjenning", {}).get("tekniskeData", {})
        ...
        forstegangsdato = kjoretoy_data.get("forstegangsregistrering", {}).get("registrertForstegangNorgeDato", "")
        arsmodell = forstegangsdato[:4] if forstegangsdato and len(forstegangsdato) >= 4 else "Unknown"
        ...
        return CarBase(
            ...
            chassisnummer=kjoretoy_data.get("kjoretoyId", {}).get("understellsnummer", "Unknown"),
            drivstoff=miljo_first.get("drivstoffKodeMiljodata", {}).get("kodeNavn", "Unknown"),
            ...
            antallseter=teknisk_data.get("persontall", {}).get("sitteplasserTotalt", 0),
            antalldorer=dorer,
            karosseri=karosseri.get("karosseritype", {}).get("kodeNavn", "Unknown"),
            eukontrollfrist=kjoretoy_data.get("periodiskKjoretoyKontroll", {}).get("kontrollfrist", "Unknown"),
